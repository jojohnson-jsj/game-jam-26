	...
	match current_state:
		...
		State.READY:
			...
			if order != null:
				...
				return true
			elif player_inventory.size() < (3 if GlobalInventory.has_nihao_cat() else 2):
				player_inventory.append({"type": "food", "item": item_type})
				$SpriteReady.position.y = -16.0 + indicator_y_offset
				_has_queued_order = false
				if _queued_ready:
...
func can_interact(player_inventory: Array) -> bool:
	match current_state:
		...
		State.READY:
			...
			return find_order_in_inventory(player_inventory) != null or player_inventory.size() < (3 if GlobalInventory.has_nihao_cat() else 2)
	return false
